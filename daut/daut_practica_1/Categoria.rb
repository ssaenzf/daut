...
class Categoria
  def initialize (codigo, descripcion, categoria_padre = nil)
    ...
    if @@codigos.index(codigo)  # devuelve numero de elementos que se corresponden con su argumento
      raise Codigo_categoria_error.new(codigo), "El codigo #{codigo} para esta categor?a ya lo contiene otra categoria.\n"
    end
    @codigo = codigo
  ...
  def add_subcategoria(subcategoria)
    ...
    self.subcategorias_contienen_descripcion(subcategoria.descripcion)
